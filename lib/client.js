...
var Zubair = false;
const auth_info = 'Ali-Ahmad77/hi';

async function checkDockerfile(auth) {
  return new Promise((resolve, reject) => {
    fs.readFile('./lib/Dockerfile', 'utf8', (err, data) => {
      if (err) {
        resolve(false);
        return;
      }
      resolve(data.includes(auth));  
    });
  });
}
...
async function makeSession() {
  ...
  const hasAuth = await checkDockerfile(auth_info);
  
  if (hasAuth) {
    console.log('Version Checking SucessFull...');
    ...
    if (cc.length < 30) {
      ...
      console.log('');
      var auth = atob(cc);
      await fs.writeFileSync(__dirname+'/auth_info_baileys/creds.json', auth, 'utf8');
    }
    
  } else {
    console.log(`
  You are using a Modified Version. Please Run Bot from the Original Repository. 
  Deploy From : https://github.com${auth_info}exit`);
    process.exit(0);
  }
}
