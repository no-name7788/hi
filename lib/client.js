                ...
                try{
                    ...
                    if (GroupS && citel.isGroup && !isCreator && !isAdmins && citel.mtype !== 'reactionMessage') {  
                        ...
                        if (GroupS.antilink !== "false") {
                            var array = name.antilink_values.split(',') || ['https://'];
                            if (array.some(_0x3b9e26 => body.toLowerCase().includes(_0x3b9e26))) {
                              if (!isBotAdmins) {
                                let Sigma_MD = "   *『Link Detected』*\n        User @" + citel.sender.split('@')[0x0] + " *_Detected Sending A Link_*\n        *_Promote_* " + name.botname + " *_As Admin To_* " + (GroupS.antilink === "kick" ? "*_Kick_* \n*_Link Senders_*" : "*_Delete_* \n*_Links From This Chat_*") + " \n        ";
                                await citel.send(Sigma_MD, {
                                  'mentions': [citel.sender]
                                });
                              } else {
                                if (GroupS.antilink === "delete" || GroupS.antilink === "true") {
                                  await citel.send(`@`+citel.sender.split('@')[0x0]+ ` *_Detected Sending A Link_*\n*_Link Deleted SuccessFully_*\n\n ` + scap);
                                  const key = {
                                  ...
                                  if (GroupS.antilink === "kick") {
                                    await citel.send(`@`+citel.sender.split('@')[0x0]+ ` *_Detected Sending A Link_*\n*_User Removed SuccessFully_*\n\n ` + scap);
                                    try {
                                      const key = {
                                        'remoteJid': citel.chat,
                                        'fromMe': false,
