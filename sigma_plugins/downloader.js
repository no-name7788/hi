...
Module_Exports({
 ...
 if (status) return await Void.sendMessage(citel.chat, {video : {url : video } , caption : name.caption } , {quoted : citel });
...
Module_Exports({
    ...
    let kingpath = (await fetch(url, { method: 'HEAD' })).headers.get('content-disposition').match(/attachment; kingpath=(.*)/)[1]
    //citel.send(`✳️ Wait, sending repository.. \n` + kingpath.toString() )
	await Void.sendMessage(citel.chat , {document : { url : url }, fileName:  kingpath,mimetype: 'application/zip',  })
...
Module_Exports({
            kingcmd: "play",
            shortcut: ["music"],
            infocmd: "Sends info about the query(of youtube video/audio).",
            kingclass: "downloader",
            kingpath: __filename,
            use: '<faded-Alan walker.>',
        },
